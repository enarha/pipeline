...
func (g *reconcilerReconcilerGenerator) GenerateType(c *generator.Context, t *types.Type, w io.Writer) error {
	...
	m := map[string]interface{}{
		...
		"metav1FieldValidationStrict": c.Universe.Constant(types.Name{
			Package: "k8s.io/apimachinery/pkg/apis/meta/v1",
			Name:    "FieldValidationStrict",
		}),
...
//
// IMPORTANT: Server-side apply behaves differently than merge patch when the target object
// no longer exists (e.g. stale cache):
// - Merge patch: Returns 404 NotFound (hard failure, no retry)
// - Server-side apply: May attempt object creation, potentially causing:
//   * Admission webhook rejections (soft failure, retry)
//   * RBAC denials if CREATE permissions missing (soft failure, retry)
//
// Controller implementations should be aware of this behavioral difference and may want to:
// - Handle creation-related errors as permanent failures
// - Implement object existence checks before applying
// - Ensure admission webhooks handle SSA creation attempts gracefully to prevent retries.
func (r *reconcilerImpl) updateFinalizersFilteredServerSideApply(ctx {{.contextContext|raw}}, resource *{{.type|raw}}, desiredFinalizers {{.setsString|raw}}) (*{{.type|raw}}, error) {
	...
	if desiredFinalizers.Has(r.finalizerName) {
		...
		// Get the GVK from the client's scheme
		...
		patchOpts := {{.metav1PatchOptions|raw}}{
			...
			FieldValidation: {{.metav1FieldValidationStrict|raw}},
		}
...
func (r *reconcilerImpl) clearFinalizer(ctx {{.contextContext|raw}}, resource *{{.type|raw}}, reconcileEvent {{.reconcilerEvent|raw}}) (*{{.type|raw}}, error) {
	...
	if err != nil {
		// Check if the resource still exists by querying the API server
		// This follows the same pattern as updateStatus in the reconciler
		logger := {{.loggingFromContext|raw}}(ctx)
		...
		if {{.apierrsIsNotFound|raw}}(getErr) {
			...
			logger.Debugw("Resource no longer exists while clearing finalizers (could happen during deletion)", 
